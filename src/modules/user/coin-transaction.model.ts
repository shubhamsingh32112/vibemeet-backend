...
export interface ICoinTransaction extends Document {
  ...
  source: 'manual' | 'payment_gateway' | 'admin' | 'video_call' | 'chat_message' | 'creator_task' | 'withdrawal' | 'welcome_bonus'; // Source of the transaction
...
const coinTransactionSchema = new Schema<ICoinTransaction>(
  {
    ...
    source: {
      type: String,
      enum: ['manual', 'payment_gateway', 'admin', 'video_call', 'chat_message', 'creator_task', 'withdrawal', 'welcome_bonus'],
      default: 'manual',
    },
    description: {
      type: String,
