...
export interface ICoinTransaction extends Document {
  ...
  source: 'manual' | 'payment_gateway' | 'admin' | 'video_call' | 'chat_message' | 'creator_task' | 'withdrawal' | 'welcome_bonus' | 'referral_reward' | 'moment_purchase' | 'moment_earnings' | 'moment_upload_reward' | 'vip_moment_free' | 'vip_membership'; // Source of the transaction
...
const coinTransactionSchema = new Schema<ICoinTransaction>(
  {
    ...
    source: {
      type: String,
      enum: ['manual', 'payment_gateway', 'admin', 'video_call', 'chat_message', 'creator_task', 'withdrawal', 'welcome_bonus', 'referral_reward', 'moment_purchase', 'moment_earnings', 'moment_upload_reward', 'vip_moment_free', 'vip_membership'],
      default: 'manual',
    },
    description: {
      type: String,
