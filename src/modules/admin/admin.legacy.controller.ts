...
import {
  getLatestSourceOfTruthReport,
  runSourceOfTruthReconciliation,
} from '../system/source-of-truth.service';
...
// ══════════════════════════════════════════════════════════════════════════
// GET /admin/system/drift — Source-of-truth drift diagnostics
// Optional query: ?runNow=true to trigger reconciliation before reading report.
// ══════════════════════════════════════════════════════════════════════════
export const getSourceOfTruthDrift = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (runNow) {
      await runSourceOfTruthReconciliation('admin_manual_diagnostics');
    }

    const latest = await getLatestSourceOfTruthReport();
    res.json({
      success: true,
      data: {
    ...
    console.error('❌ [ADMIN] Source-of-truth drift diagnostics error:', error);
    res.status(500).json({ success: false, error: 'Internal server error' });
  }
};
