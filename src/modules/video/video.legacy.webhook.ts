...
async function handleSessionEnded(payload: StreamVideoWebhookPayload): Promise<void> {
  ...
  // Add call activity in chat so users can see completed video calls in the thread.
  try {
    const [callerUser, creatorUser, creatorProfile] = await Promise.all([
      User.findById(call.callerUserId),
      User.findById(call.creatorUserId),
      Creator.findOne({ userId: call.creatorUserId }),
    ]);

    const userFirebaseUid = callerUser?.firebaseUid;
    ...
    if (userFirebaseUid && creatorFirebaseUid) {
      const creatorName =
        ...
        creatorUser?.email ||
        'Creator';
      await postCallActivityToChat({
        callId,
        userFirebaseUid,
        creatorFirebaseUid,
        creatorName,
        durationSeconds: call.billedSeconds ?? call.durationSeconds ?? 0,
        coinsSpent: Math.ceil(call.userCoinsSpent || 0),
      });
    }
  } catch (chatErr) {
    console.error(`⚠️  [VIDEO WEBHOOK] Failed to post call activity in chat for ${callId}:`, chatErr);
  }
